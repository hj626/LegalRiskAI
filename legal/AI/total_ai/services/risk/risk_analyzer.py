# services/risk/risk_analyzer.py
"""
법률 위험도 분석 서비스
- MultiTaskLegalBERT: 승소율, 형량, 벌금, 위험도 예측
- Gemini: 상세 피드백 생성
"""
import os
...
# 모델 파일 경로
MODEL_PATH = os.path.join(os.path.dirname(__file__), "../../models/multi_task_bert")
...
class LegalAnalyzer:
    ...
    def predict_bert(self, text: str) -> Dict[str, Any]:
        ...
        # 사건 유형 분류 (classifier 사용)
        try:
            from classifier.bert_classifier import get_classifier
            classifier = get_classifier()
            case_type, _, _ = classifier.classify(text)
        except Exception:
            case_type = "일반 사건"
    ...
    def generate_feedback(self, story: str, bert_results: Dict) -> str:
        """Gemini로 상세 피드백 생성"""
        # case_type이 없으면 기본값 사용
        case_type = bert_results.get('case_type', '일반 사건')
